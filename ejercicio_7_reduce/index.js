...
const sumaNotas = exams.reduce(
  (accumulador, currentExam) => accumulador + currentExam.score,
  0
)
...
const totalAprobados = exams.reduce((accumulator, currentExam) => {
  return currentExam.score >= 5 ? accumulator + currentExam.score : accumulator
}, 0)
...
const mediaNotas =
  exams.reduce((accumulador, notas) => accumulador + notas.score, 0) /
  exams.length
